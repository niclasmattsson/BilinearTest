...
function makevariables(model, params)
    ...
    @variables model begin
        ...
        BilinearVariable[p in PLANT, s in SLICE] >= 0
    end

    return (; Systemcost, CO2emissions, FuelUse, Totalwaste, HeatProduction, Invest, BilinearVariable)
end
...
function makeconstraints(model, vars, params)
    ...
    @unpack Systemcost, CO2emissions, FuelUse, Totalwaste, HeatProduction, Invest, BilinearVariable = vars
    ...
    @constraints model begin
        ...
        WeirdBilinearStuff[p in PLANT, s in SLICE],
            BilinearVariable[p,s] == HeatProduction[p,s] * Invest[p]
        ...
        Totalcosts,
            Systemcost == 1/1000 * (
                ...
                0.1 * sum(BilinearVariable[p,s] for p in PLANT, s in SLICE)
            )
    end   #constraints
